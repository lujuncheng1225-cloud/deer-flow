...
def test_normalize_postgres_conn_string_strips_sqlalchemy_driver_suffix():
    assert (
        normalize_postgres_conn_string("postgresql+psycopg://user:pass@example:5432/db")
        == "postgresql://user:pass@example:5432/db"
    )
    assert (
        normalize_postgres_conn_string("postgresql+asyncpg://user:pass@example:5432/db")
        == "postgresql://user:pass@example:5432/db"
    )
    assert (
        normalize_postgres_conn_string("postgres+psycopg://user:pass@example:5432/db")
        == "postgresql://user:pass@example:5432/db"
    )
...
def test_normalize_postgres_conn_string_leaves_native_psycopg_dsn_unchanged():
    assert (
        normalize_postgres_conn_string("postgresql://user:pass@example:5432/db")
        == "postgresql://user:pass@example:5432/db"
    )
