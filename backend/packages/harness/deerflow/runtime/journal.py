...
- on_chat_model_start captures structured prompts as llm_request (OpenAI format)
  and provides a fallback for user-authored input not present in the raw run input.
...
class RunJournal(BaseCallbackHandler):
    ...
    def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        Raw run input is the canonical UI message source. This callback only
        captures user-authored input that can exist solely in model context,
        such as a hidden HumanInputCard response.
        """
        rid = str(run_id)
        ...
        # Ordinary prompt-level input is diagnostic because middleware may have
        # transformed it. Hidden HumanInputCard responses are user-authored and
        # must remain message evidence even though they are not rendered directly.
        caller = self._identify_caller(tags)
            ...
            for batch in reversed(messages):
                for m in reversed(batch):
                    if _should_persist_human_input_message(m):
                        ...
                        is_hidden_response = m.additional_kwargs.get("hide_from_ui") is True
                        self._put(
                            event_type="llm.human.input",
                            category="message" if is_hidden_response else "trace",
                            content=m.model_dump(),
                            metadata={"caller": caller},
                        )
                        if is_hidden_response:
