...
- Competitor subscription-price requests enter the Commodity Center gateway preflight whenever a trusted app code is recognized. If platform is absent, the gateway queries `2=iOS/macOS`, `3=Google`, and `4=PC/Web`; if region is absent, it omits that parameter so the internal API returns its current regional coverage. Public-web fallback is permitted only after every internal platform query succeeds with zero records; any failed internal query remains an explicit availability gap.
...
CORS is same-origin by default when requests enter through nginx on port 2026. Split-origin or port-forwarded browser clients must opt in with `GATEWAY_CORS_ORIGINS` (comma-separated exact origins); Gateway `CORSMiddleware` and `CSRFMiddleware` both read that variable so browser CORS and auth-origin checks stay aligned. For an authenticated `/api/v1/auth/me` request whose persistent `access_token` survives without its `csrf_token`, `CSRFMiddleware` restores the readable CSRF cookie; failed authentication probes do not mint one.
