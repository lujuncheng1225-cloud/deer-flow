...
The unified nginx endpoint is same-origin by default and does not emit browser CORS headers. If you run a split-origin or port-forwarded browser client, set `GATEWAY_CORS_ORIGINS` to comma-separated exact origins such as `http://localhost:3000`; the Gateway then applies the CORS allowlist and matching CSRF origin checks. Browser clients also recover a missing CSRF cookie through the authenticated `/api/v1/auth/me` probe before their next state-changing request, so an otherwise valid persistent session does not need to sign in again. DDGS search can optionally use a shorter per-engine `timeout` and a bounded `direct_site_first` path for brand pricing/official-site queries; Jina `web_fetch` can likewise enable `direct_first`, use Jina only when direct extraction is insufficient, and select pure-Python readability extraction with `use_readability_js: false` when runtime Node dependency installation is undesirable.

Interactive research uses risk-aware clarification: an obvious typo or awkward phrase does not interrupt a low-risk lookup when one entity or intent is a strong match. The lead agent states its assumption and continues; it still asks before risky actions or when multiple plausible entities remain.
